...
def test_runtime_integration():
    ...
    print("🚀 Starting Mailgun MCP Runtime Integration tests...")

    # Create a temporary cache directory
    temp_cache_dir = Path(tempfile.mkdtemp(prefix="mailgun_test_"))
    ...
    try:
        ...
        server.cache_manager.cache_file = temp_cache_dir / "mailgun-mcp_cache.json"
        ...
        assert len(health_response.components) == 3  # api_key, cache, snapshot
...
if __name__ == "__main__":
    success = test_runtime_integration()

    if success:
        print("\n🎉 Mailgun MCP Runtime Integration is fully operational!")
        sys.exit(0)
    else:
        print("\n❌ Mailgun MCP Runtime Integration tests failed!")
        sys.exit(1)
